...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Банка", text = "Акаунт не съществува!", icon = "fas fa-university", color = "#ff0000", }, })
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Банка", text = "&#36;"..amount.." добавени към вашия акаунт!", icon = "fas fa-university", color = "#8c7ae6", }, })
    ...
    if not delete then 
        if src ~= MyPlayerId then
            SendNUIMessage({
                action = "PhoneNotification",
                PhoneNotify = {
                    title = "Нов туит: (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")",
                    text = "Нов туит, както беше публикуван.",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Мейл",
            text = "Нов имейл от: "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Реклама",
            text = "Публикувана нова обява: "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Отдел за фактуриране',
            subject = 'изплатена фактура',
            message = 'Фактурата е платена от '..name..' В количеството на $'..data.amount,
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Отдел за фактуриране',
            subject = 'Фактурата е отхвърлена',
            message = 'Фактурата е отхвърлена '..name..' В количеството на $'..data.amount,
        })
    end
end)
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        QBCore.Functions.Notify("Звука е изключен", "success")
        cb(true)
    else
        CallVolume = 0.2
        QBCore.Functions.Notify("Звука е включен", "error")
        cb(false)
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Съобщения",
                        text = "Ново съобщение: "..IsNumberInContacts(SenderNumber),
                        icon = "fas fa-comment",
                        color = "#25D366",
                        timeout = 1500,
                    },
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Съобщения",
                        text = "Прати съобщение на себе си?",
                        icon = "fas fa-comment",
                        color = "#25D366",
                        timeout = 4000,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Банка",
                text = "$"..amount.." премахнати от балансът ви!",
                icon = "fas fa-university",
                color = "#069c0b",
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Споменаха ви!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
